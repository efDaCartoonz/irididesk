...
if exist \"{tmp_path}\\{app_name} Tray.lnk\" del /f /q \"{tmp_path}\\{app_name} Tray.lnk\"
    ...
    // iRidi build:
    // Server parameters are compiled into the binary.
    // Do not persist key/custom-rendezvous-server/api-server to RustDesk.toml.
    // This prevents the installed client from exposing relay/rendezvous settings in config.
    if let Some(_lic) = get_license() {
        // no-op
    }
