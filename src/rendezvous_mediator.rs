...
impl RendezvousMediator {
        ...
        loop {
            let mut update_latency = || {
                ...
                if (latency - old_latency).abs() > n || old_latency <= 0 {
                    ...
                    log::debug!("Latency of <server>: {}ms", latency as f64 / 1000.);
                    old_latency = latency;
                }
            };
            select! {
    ...
    async fn handle_resp(
        ...
        match msg {
            Some(rendezvous_message::Union::RegisterPeerResponse(rpr)) => {
                update_latency();
                if rpr.request_pk {
                    log::info!("request_pk received from rendezvous server");
    ...
    pub async fn start(server: ServerPtr, host: String) -> ResultType<()> {
        log::info!("start rendezvous mediator of <server>");
    ...
    async fn create_relay(
        ...
        log::info!(
            "create_relay requested from {:?}, relay_server: <server>, uuid: {}, secure: {}",
            peer_addr,
            uuid,
            secure,
        );
    ...
    async fn handle_uuid_mismatch(&mut self, socket: Sink<'_>) -> ResultType<()> {
        {
            ...
            if solving.is_empty() || *solving == self.host {
                log::info!("UUID_MISMATCH received from rendezvous server");
